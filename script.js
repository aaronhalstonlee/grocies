...
// ---- Modals ----
function close(id){document.getElementById(id).classList.add('h')}
...
function parseCSV(txt){
  ...
  const start=(first==='meal'||first==='name'||first==='meal name'||first==='dinner')?1:0;
  const out=[];
  for(let i=start;i<rows.length;i++){
    const [rn='',rc='',rcu='']=rows[i];
    const name=rn.trim();if(!name)continue;
    const cl=rc.trim().toLowerCase();
    let primary='other';
    for(const [k,aliases] of Object.entries(ALIASES)){
      if(aliases.some(a=>cl.includes(a))){primary=k;break}
    }
...
function prevImport(){
  ...
  if(!iParsed.length){el.textContent='No valid rows found — check the format above.';return}
  const cnt={};iParsed.forEach(m=>{cnt[m.primary]=(cnt[m.primary]||0)+1});
  el.innerHTML=`<strong>${iParsed.length} meals found:</strong> `+
    Object.entries(cnt).map(([k,v])=>`${PR[k]?.ic} ${v} ${PR[k]?.lb}`).join(' · ')+
    `<br><small>Duplicates (by name) will be skipped.</small>`;
}
function doImport(){
  if(!iParsed.length){prevImport();if(!iParsed.length)return}
  const existing=new Set(meals.map(m=>m.name.toLowerCase()));
  let added=0;
  iParsed.forEach(m=>{if(existing.has(m.name.toLowerCase()))return;meals.push({id:uid(),...m});added++});
  save();rAll();close('ov-import');
  toast(`${added} meal${added!==1?'s':''} imported ✓`);
}
